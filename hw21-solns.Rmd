...
Note the following won't evaluate in R:
```{r}
num =  (1/xbar)^n*exp(-n) 
denom = my_log_lik(a_MLE, l_MLE, n) 
-2*log(num/denom)
```
So we have to answer this question analytically and note that $-2 \ln (0) \equiv \infty$. 
...
Again, this won't evaluate in R:
```{r eval=FALSE}
