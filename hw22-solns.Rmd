...
## The standard error if we treat DE and PA prices as coming from independent populations is s_p*sqrt(1/12 + 1/12)
## Note: you have to be really careful with partheneses in the following code 
s_p = ( sum((mydata$DE_TotalWine-mean(mydata$DE_TotalWine))^2) + sum((mydata$PA_RiteBuy-mean(mydata$PA_RiteBuy))^2) ) / 12+12-2
SE_ind = s_p*sqrt((1/12) + (1/12))
SE_ind

## The standard error if we treat DE and PA prices as linearly dependent is s_diff * sqrt(1/12)
s_diff = (1/11) * sum((mydata$Difference - mean(mydata$Difference))^2)
...
```{r}
mean(mydata$Difference) - ( qt(0.01/2, df=11) * sd(mydata$Difference)/sqrt(12)  )
mean(mydata$Difference) + ( qt(0.01/2, df=11) * sd(mydata$Difference)/sqrt(12)  )
```
...
Using the information from the problem, we can use $Y$ as our test statistic and the rejection region will be $\{(PA_{diff,i},DE_{diff,j}) :  \mid (PA_{diff,i} - DE_{diff,j} \mid \geq c_{\alpha} \}$ where $c_{\alpha}$ solves: $Pr( \mid (PA_{diff,i} - DE_{diff,j} \mid \geq c_{\alpha}) = 0.01$. Since the problem asks us to find a p-value and we have a significance level, we don't actually need to calculate $c_{\alpha}$ and can instead directly calculate the p-value which is  
\begin{align*}
Pr( \mid (PA_{diff,i} - DE_{diff,j} \mid \geq 10 \mid H_0: \theta = 1/2) &= Pr( \mid d_i \mid \geq 10 \mid H_0: \theta = 1/2) \\
&= Pr(Y \leq 2 \mid H_0: \theta = 1/2) + Pr(Y \geq 10 \mid H_0: \theta = 1/2) \\ 
&= Pr(Y=0 \mid H_0: \theta = 1/2) + Pr(Y=1 \mid H_0: \theta = 1/2) + Pr(Y=2 \mid H_0: \theta = 1/2) + \\
&\quad Pr(Y=10 \mid H_0: \theta = 1/2) + Pr(Y=11 \mid H_0: \theta = 1/2) + Pr(Y=12 \mid H_0: \theta = 1/2)
\end{align*}
...
t.test(desi, plac, alternative="two.sided", paired=TRUE, conf.level = 0.95)
```
...
```{r}
coc_fisher_table = matrix(c(10,24-10,18,24-18), nrow=2, byrow=TRUE)  ## Desi is first row, Lithium is second row, relapse=TRUE is the first column, relapse=FALSE is the second column
chisq.test(coc_fisher_table, simulate.p.value = TRUE)
```
The p-value of a Fisher's exact test for an association between the drug treatment type and whether or not a patient relapses is $0.039$
